...
impl Gpio<Uninitialized> {
    ...
    pub fn into_alt(self, function: Function) -> Gpio<Alt> {
        ...
        let (y, x) = (self.pin / 10, self.pin % 10);
        let fsel_y = &mut self.registers.FSEL[y as usize];
        fsel_y.write((fsel_y.read() & !(FUNCTION_MASK << (x * 3))) | func << (x * 3));
        Gpio {
            registers: self.registers,
            pin: self.pin,
            _state: PhantomData,
...
impl Gpio<Output> {
    ...
    pub fn set(&mut self) {
        let (y, x) = (self.pin / 32, self.pin % 32);
        let set_y = &mut self.registers.SET[y as usize];
        set_y.write(1 << x);
    }
    ...
    pub fn clear(&mut self) {
        let (y, x) = (self.pin / 32, self.pin % 32);
        let clr_y = &mut self.registers.CLR[y as usize];
        clr_y.write(1 << x);
    }
}
...
impl Gpio<Input> {
    ...
    pub fn level(&mut self) -> bool {
        let (y, x) = (self.pin / 32, self.pin % 32);
        let lev_y = &mut self.registers.LEV[y as usize];
        lev_y.read() & (1 << x) != 0
    }
}
